...
path_txt= str(caminhoXml)+"\Cadastro_Fornecedores.txt" #variável que armazenará o caminho onde ficará o arquivo txt com os valores das tags solicitadas para todos os XML
...
tags = ["CNPJ","xNome","xFant","xLgr","nro","xBairro","cMun","xMun","UF","CEP","cPais","xPais","fone","IE"] #lista4 com as tags solicitadas referente aos cadastros de fornecedores
...
def save_archive(ext): #função para salvar os valores das tags em formato .csv e .txt
    ...
    global count
    global armazena_xml
    with open(armazena_xml[conta]+"_FORNECEDORES_"+str(ext),"w") as att_file:
...
while conta <= len(armazena_xml)-1: #será iniciado o loop onde os arquivos XML serão lidos e verificados as tags solicitadas
    ...
    for elem in root.findall(".//{http://www.portalfiscal.inf.br/nfe}emit//"): #laço for para iniciar a verificação das tags solicitadas, encontrando todas as subtags que estão dentro da tag "emit"
...
print("Verifique na pasta "+caminhoXml+" os arquivos com os XML indentados em TXT e o arquivo Cadastro_Fornecedores.txt com as informações solicitadas \n \n \n \n")
