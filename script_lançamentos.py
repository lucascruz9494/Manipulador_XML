...
path_txt= str(caminhoXml)+"\Lançamentos.txt" #variável que armazenará o caminho onde ficará o arquivo txt com os valores das tags solicitadas para todos os XML
...
tags = ["mod","serie","nNF","dhEmi","CNPJ"] #lista4 com as tags solicitadas referente aos lançamentos
...
def save_archive(ext): #função para salvar os valores das tags em formato .csv e .txt
    ...
    global count
    global armazena_xml
    with open(armazena_xml[conta]+"_LANÇAMENTOS_"+str(ext),"w") as att_file:
...
while conta <= len(armazena_xml)-1: #será iniciado o loop onde os arquivos XML serão lidos e verificados as tags solicitadas
    ...
    tree = ET.parse(armazena_xml[conta]) #variável necessária para analisar o xml armazenado na lista, porém para verificação posterior das tags e atributos
    ...
    for elem in root.findall(".//{http://www.portalfiscal.inf.br/nfe}ide//"): #laço for para iniciar a verificação das tags solicitadas, encontrando todas as subtags que estão dentro da tag "ide"
        ...
        text_info.append(text_ide) #armazenamento do valor da tag 
        
        tags_info.append(tag_ide) #armazenamento do nome da tag

    while segue<=len(tags_info)-1: #laço while para comparar as tags encontradas no XML com as tags armazenadas na lista tags
...
print("Verifique na pasta "+caminhoXml+" os arquivos com os XML indentados em TXT e o arquivo Lançamentos.txt com as informações solicitadas \n \n \n \n")
